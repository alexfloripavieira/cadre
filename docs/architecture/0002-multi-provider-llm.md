...
Cadre's value proposition is reliability for AI engineering agents. Reliability in
...
- **Local/self-hosted support.** Customers running on-prem or behind strict data
  boundaries need to point Cadre at vLLM, Ollama, or an internal OpenAI-compatible
  endpoint with zero code changes.

Building a provider abstraction layer from scratch is a significant engineering
effort that does not differentiate Cadre — the differentiation is the reliability
layer on top, not the SDK adapters underneath.

## Decision

Use **LiteLLM** as the single provider-agnostic LLM client for the Cadre runtime.

- All LLM calls inside `services/runtime/cadre/providers/` go through LiteLLM's
...
- The fallback matrix (ADR TBD) is implemented on top of LiteLLM's `completion_with_fallbacks`,
  wrapped by Cadre's retry-budget and doom-loop detection.
- Observability (token counts, cost, latency) is captured from LiteLLM's response
  metadata and emitted into the Cadre SEP log.
...
- Cadre focuses engineering effort on the reliability layer, not SDK plumbing.
...
- LiteLLM becomes a critical dependency; a regression or breaking change upstream can
  ...
  Cadre's golden-run suite on upgrades.
- Provider-specific features (e.g. Anthropic prompt caching, OpenAI structured outputs)
  ...
  and unsupported, Cadre may bypass LiteLLM for that specific call path and document
  the exception.
...
- **Direct SDK integration per provider.** Rejected: triples the surface area to
  maintain and forces Cadre to re-implement fallback/streaming uniformly across
  heterogeneous SDKs.
...
- **LangChain / LlamaIndex as the provider layer.** Rejected: both bring a large
  opinionated stack (chains, retrievers, agents) that conflicts with Cadre's own
  orchestration model. Cadre's orchestration is the product; we don't want to
